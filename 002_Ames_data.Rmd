...
ames %>% 
  ggplot( aes( x = Sale_Price )) +
    geom_histogram( bins = 25,
                    ...
                    fill  = "green" )
```
I prefer to use a density histoegram and too draw a smoothed  density plot with the 
histogram.  I also use the ggplot theme theme_bw().  In addition when dealing with dollar amounts greater that are in the thousands of dollars it is useful to vhnage the unit to thosuands of dollars ($k) rather than dollars($)
```{r}
...
Again we will change this by working in units of thousandas of dollars.
...
Incidentally, there is the the law of ten, which says that if the ratio of the maximum values divided by the minimum value the logarithm transformation will be useful in obtaining a symmetric distribution. (I doi';t currently have the full reference for this butit is in Applied Regression Analysis by _________.)  
...
Therefore we are going to update the Ames data set set by replacing sale proce by the logarithm of the sale proce.  Since we are using United states dollares we will use logs to the base 10.  
...
##################################################################################
# Verify the transformation was made by looking at the first 10 sale proces
##################################################################################
